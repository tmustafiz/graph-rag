...
| `search_code` | Same hybrid search, over ingested source-code entities — functions / classes / modules / methods (Python built-in; Java via the `[java]` extra, JavaScript / TypeScript via `[js]`). SQL (`[sql]`): schema DDL ingests as a `:DbTable` / `:DbColumn` / `:DbView` graph, and stored procedures / functions / packages / triggers ingest as `CodeEntity` routines with `CALLS` + `READS`/`WRITES`/`ON` edges to those tables — all walkable with `get_neighbors`. Optional `stereotype=` / `annotation=` / `module=` filters narrow to the Spring / Java-framework graph. |
| `search_policies` | Hybrid search over Checkov policy content — the fuzzy complement to `find_policies_for`. |
| `get_beans_for` / `get_endpoints` / `get_routes` / `get_message_flows` / `get_service_calls` / `get_architecture_outline` | Enterprise Java (Spring / Jakarta / Apache Camel): bean wiring (`INJECTS` / `PRODUCES` / `BINDS`, annotation- and XML-wired), MVC / JAX-RS routes, Camel routes (Java + XML + YAML DSL), in-process events + broker messaging (`PUBLISHES` / `CONSUMED_BY` / `PRODUCES_TO`), AOP advice + behavioral markers, MyBatis mapper SQL (`EXECUTES` / `ACCESSES`), and declarative HTTP clients (`@FeignClient` / `@HttpExchange` → `CALLS_SERVICE` / `RESOLVES_TO`, a cross-service call graph). `search_code` filters `stereotype=` / `annotation=` / `module=` / `route=` / `endpoint=` / `listens_to=` / `behavior=`; `get_central_code_entities` folds framework edges into PageRank. See `examples/enterprise-java/` for a runnable three-module walkthrough. Compiler-grade cross-file / library resolution is `grag-mcp ingest --scip` (a SCIP index from `scip-java`); the default static graph is build-free. |
...
| `get_neighbors` | Walk the graph from any node — Source path, Section/Chunk/PolicyRule/AgentMemory/Bean/HttpEndpoint id, CodeEntity qualified name, Module path, or Concept name — optionally filtered by relationship type (`INJECTS` / `HANDLED_BY` / `MANAGES` / `BINDS` / … all traverse). |
