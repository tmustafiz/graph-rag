...
| `get_beans_for` / `get_endpoints` | Spring / Spring Boot: a bean's `INJECTS` / `PRODUCES` / `BINDS` wiring (annotation- and XML-wired), or the Spring MVC / JAX-RS HTTP routes with their handler + module. The graph also carries Spring Data repositories + JPA entities (`MANAGES` / `PERSISTS_AS` / `RELATES_TO`), all walkable with `get_neighbors`. See `examples/spring-boot/` for a runnable two-module walkthrough. Precise cross-file / library resolution is the v0.7.0 `--scip` path; today's Java graph is best-effort static. |
