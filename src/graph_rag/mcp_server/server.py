...
INSTRUCTIONS = (
    ...
    "Section/Chunk/PolicyRule/AgentMemory/Bean/HttpEndpoint id, CodeEntity "
    "qualified_name, Module path, or Concept name). `get_central_code_entities` "
    "ranks code by PageRank over the CALLS/IMPORTS graph — use it to find "
    "what's most depended-upon (and riskiest to change) in this codebase; "
    "empty until `grag-mcp compute-centrality` has been run at least once. "
    "`cite` returns a human-readable citation string for a chunk. `ingest_path` "
    "(re-)ingests a file or directory after it changes. For an enterprise Java "
    "(Spring / Jakarta / Camel) codebase, the graph carries beans + DI, HTTP "
    "endpoints, Spring Data / JPA, application config, Camel routes, events + "
    "broker messaging, AOP + behavioral markers, MyBatis SQL, and Feign / "
    "`@HttpExchange` clients: `search_code` filters `stereotype=` / "
    "`annotation=` / `module=` / `route=` / `endpoint=` / `listens_to=` / "
    "`behavior=`; `get_beans_for`, `get_endpoints`, `get_routes`, "
    "`get_message_flows`, `get_service_calls`, `get_architecture_outline`; "
    "`get_central_code_entities` folds framework edges into PageRank. "
    "Compiler-grade cross-file / library resolution is `grag-mcp ingest "
    "--scip`. Use `remember`/`recall`/`forget` "
