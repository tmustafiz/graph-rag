...
INSTRUCTIONS = (
    ...
    "(re-)ingests a file or directory after it changes. For a Spring / Spring "
    "Boot / Jakarta codebase, use `search_code` with `stereotype=` / "
    "`annotation=` / `module=`, `get_beans_for(qualified_name)`, "
    "`get_endpoints(path_glob?, http_method?, module?)`, and `get_neighbors` "
    "over `INJECTS` / `HANDLED_BY` / `MANAGES` / `BINDS` / `PERSISTS_AS` / "
    "`RELATES_TO`; precise cross-file/library resolution is the v0.7.0 `--scip` "
    "path. Use `remember`/`recall`/`forget` "
