...
class IngestionPipeline:
    ...
    what would change without generating embeddings or writing to Neo4j.
    """
    ...
    def run(self, path: Path, dry_run: bool = False) -> list[IngestionResult]:
        if path.is_file():
            ...
            return [self._ingest_one(path, parser, dry_run)]

        pairs = sorted(
            ((file, self._registry.for_path(file)) for file in path.rglob("*") if file.is_file()),
            key=lambda pair: pair[0],
        )
        return [
            self._ingest_one(file, parser, dry_run) for file, parser in pairs if parser is not None
        ]
