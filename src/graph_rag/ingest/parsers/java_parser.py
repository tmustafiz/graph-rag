...
class JavaParser:
    ...
    def parse(self, path: Path) -> ParsedDocument:
        ...
        # Imported here, not at module scope: `CamelJavaDslExtractor` subclasses
    ...
    # -- messaging / event flow sites -------------------------------

    @classmethod
    def _collect_message_sites(
        cls,
        type_nodes: list["Node"],
        content: bytes,
        imported_types: dict[str, str],
        ...
        """Raw event/messaging sites the annotation model can't see:
        `@EventListener` methods (event type resolved from `classes=` or the
        first parameter), `publishEvent(...)` call sites (event type from the
        argument's `new X(...)` or a resolvable parameter), and
        `KafkaTemplate` / `RabbitTemplate` / `JmsTemplate` `.send(...)` /
        `.convertAndSend(...)` call sites with a literal destination.
        """
